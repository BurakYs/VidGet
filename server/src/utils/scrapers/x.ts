...
export default class XScraper {
    ...
    static async scrapePost(postUrL: string) {
        ...
        const tombstone = postData.tombstone && tombstoneErrors.find((error) => postData.tombstone.text.text.startsWith(error.prefix));
        if (tombstone || postData.tombstone)
            throw new ScraperError(tombstone?.description || 'Failed to fetch post data');
